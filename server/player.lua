...
function QBCore.Player.Save(source)
    ...
    if PlayerData then
        ...
        if GetResourceState(QBConfig.Inventory) ~= 'missing' then exports[QBConfig.Inventory]:SaveInventory(source) end
...
function QBCore.Player.SaveOffline(PlayerData)
    if PlayerData then
        ...
        if GetResourceState(QBConfig.Inventory) ~= 'missing' then exports[QBConfig.Inventory]:SaveInventory(PlayerData, true) end
...
function QBCore.Player.SaveInventory(source)
    if GetResourceState(QBConfig.Inventory) == 'missing' then return end
    exports[QBConfig.Inventory]:SaveInventory(source, false)
end

function QBCore.Player.SaveOfflineInventory(PlayerData)
    if GetResourceState(QBConfig.Inventory) == 'missing' then return end
    exports[QBConfig.Inventory]:SaveInventory(PlayerData, true)
end

function QBCore.Player.GetTotalWeight(items)
    if GetResourceState(QBConfig.Inventory) == 'missing' then return end
    return exports[QBConfig.Inventory]:GetTotalWeight(items)
end

function QBCore.Player.GetSlotsByItem(items, itemName)
    if GetResourceState(QBConfig.Inventory) == 'missing' then return end
    return exports[QBConfig.Inventory]:GetSlotsByItem(items, itemName)
end

function QBCore.Player.GetFirstSlotByItem(items, itemName)
    if GetResourceState(QBConfig.Inventory) == 'missing' then return end
    return exports[QBConfig.Inventory]:GetFirstSlotByItem(items, itemName)
end
